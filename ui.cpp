...
void refresUI() {
  ...
  if (digitalRead(WIO_KEY_C) == LOW && ui.selected_display != DISPLAY_DISCO ) {
...
#else 
         ui.selected_display = DISPLAY_DISCO;
         enterDisco();
#endif        
         forceRefresh = true;
         break;
    ...
    switch ( ui.selected_display ) {
      case DISPLAY_DISCO:
         if ( state.discoveryState == DISCO_END || state.discoveryState == DISCO_READY || state.discoveryState == DISCO_NONE ) {
           ui.previous_display = ui.selected_display;
           #ifdef WITH_GPS
              ui.selected_display = DISPLAY_GPS;
           #else         
...
void refreshDisco() {
  if ( ui.previous_display != ui.selected_display ) {
    ...
    tft.setFreeFont(FF25);    
    tft.setTextColor(TFT_WHITE);
    tft.drawString("DISCO",HIST_X_OFFSET,HIST_Y_OFFSET-18,GFXFF);
    tft.drawRoundRect(HIST_X_OFFSET,HIST_Y_OFFSET,HIST_X_SIZE,HIST_Y_SIZE,R_SIZE,TFT_WHITE);
    ui.previous_display = ui.selected_display;
  }
  // Clear the whole area, all is displayed back
  ...
  switch ( state.discoveryState ) {
    ...
    case DISCO_READY:
      tft.setFreeFont(FF25);    
      tft.setTextColor(TFT_WHITE);
      tft.drawString("PRESS BUTTON TO START",DISCO_X_OFFSET,DISCO_y_OFFSET,GFXFF);
      break;
    case DISCO_WAIT:
      tft.setFreeFont(FF25);    
      tft.setTextColor(TFT_WHITE);
      tft.drawString("WAIT FOR DUTY CYCLE & GPS",DISCO_X_OFFSET-15,DISCO_y_OFFSET,GFXFF);
      break;
    case DISCO_TX: {
    ...
    case DISCO_END: {
        ...
        char url[128];
        sprintf(url,"https://dev.disk91.com/ft/r/?x=%02X%02X%02X%02X%02X", loraConf.deveui[3],loraConf.deveui[4], loraConf.deveui[5], loraConf.deveui[6], loraConf.deveui[7]);
        ...
        for (uint8_t y = 0; y < qrcode.size; y++) {
          for (uint8_t x = 0; x < qrcode.size; x++) {
            if ( qrcode_getModule(&qrcode, x, y) ) {
              ...
              tft.fillRect(10+xs+(4*x),10+ys+(4*y),4,4,TFT_BLACK);
            }
          }
        }
      }
